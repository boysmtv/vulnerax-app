...
export default function BrowserExt() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey:['browser-extensions', eff],
    queryFn: async()=> (await api.get('/api/v1/browser-extensions', { params: { projectId: eff, organizationId: eff } })).data.data,
    enabled: !!eff
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        <div><h1 className="text-2xl font-bold">Browser Extension</h1><p className="text-sm text-slate-500">Manifest â€¢ Permissions â€¢ Content scripts â€¢ Messaging</p></div>
      ...
      {isLoading ? <div className="text-sm text-slate-500">Loading...</div> : (
        <div className="bg-white border rounded-xl overflow-hidden">
          ...
          {list.length===0 && <div className="p-8 text-center text-sm text-slate-500">No data â€” Browser Extension will appear here after sync/scan.</div>}
          <div className="p-3 text-xs text-slate-500 border-t bg-slate-50">API: GET /api/v1/browser-extensions â€¢ POST â€¢ GET /{'{id}'} â€¢ Auto-mapped to PRD â€¢ Browser Extension</div>
        </div>
      )}
    </div>
  )

...
export default function SupplyChain() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey:['provenance', eff],
    queryFn: async()=> (await api.get('/api/v1/provenance', { params: { projectId: eff, organizationId: eff } })).data.data,
    enabled: !!eff
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        <div><h1 className="text-2xl font-bold">Supply Chain</h1><p className="text-sm text-slate-500">SBOM â€¢ Provenance â€¢ Signature â€¢ Build integrity</p></div>
      ...
      {isLoading ? <div className="text-sm text-slate-500">Loading...</div> : (
        <div className="bg-white border rounded-xl overflow-hidden">
          ...
          {list.length===0 && <div className="p-8 text-center text-sm text-slate-500">No data â€” Supply Chain will appear here after sync/scan.</div>}
          <div className="p-3 text-xs text-slate-500 border-t bg-slate-50">API: GET /api/v1/provenance â€¢ POST â€¢ GET /{'{id}'} â€¢ Auto-mapped to PRD â€¢ Supply Chain</div>
        </div>
      )}
    </div>
  )

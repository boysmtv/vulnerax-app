...
export default function Cicd() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey:['cicd', eff],
    queryFn: async()=> (await api.get('/api/v1/cicd', { params: { projectId: eff, organizationId: eff } })).data.data,
    enabled: !!eff
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        <div><h1 className="text-2xl font-bold">CI/CD Security</h1><p className="text-sm text-slate-500">GitHub Actions â€¢ Jenkins â€¢ Pipeline perms â€¢ Secrets â€¢ Branch protection</p></div>
      ...
      {isLoading ? <div className="text-sm text-slate-500">Loading...</div> : (
        <div className="bg-white border rounded-xl overflow-hidden">
          ...
          {list.length===0 && <div className="p-8 text-center text-sm text-slate-500">No data â€” CI/CD Security will appear here after sync/scan.</div>}
          <div className="p-3 text-xs text-slate-500 border-t bg-slate-50">API: GET /api/v1/cicd â€¢ POST â€¢ GET /{'{id}'} â€¢ Auto-mapped to PRD â€¢ CI/CD Security</div>
        </div>
      )}
    </div>
  )

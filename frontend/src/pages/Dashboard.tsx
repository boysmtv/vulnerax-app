...
export default function Dashboard() {
  ...
  const secScore = data?.securityScore ?? 74
  const bySev = data?.bySeverity ? Object.entries(data.bySeverity).map(([name,value])=>({name,value})) : [{name:'CRITICAL',value:3},{name:'HIGH',value:17},{name:'MEDIUM',value:42},{name:'LOW',value:71}]
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        <div className="bg-white rounded-xl border p-5"><div className="text-xs text-slate-500">Critical Risk</div><div className="text-3xl font-bold text-red-600">{data?.critical ?? 3}</div><div className="text-xs text-slate-500 mt-1">{data?.kev ?? 1} KEV • {data?.internetExposed ?? 12} exposed</div></div>
        <div className="bg-white rounded-xl border p-5"><div className="text-xs text-slate-500">High Risk</div><div className="text-3xl font-bold text-orange-500">{data?.high ?? 17}</div><div className="text-xs text-slate-500 mt-1">{data?.slaBreached ?? 2} SLA breached</div></div>
        <div className="bg-white rounded-xl border p-5"><div className="text-xs text-slate-500">Assets</div><div className="text-3xl font-bold">{data?.totalAssets ?? 247}</div><div className="text-xs text-slate-500 mt-1">{data?.internetExposed ?? 12} internet-exposed</div></div>
      </div>
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-xl border p-5">
          ...
          <div className="space-y-3">
            {(data?.topRiskAssets || [{name:'Payment API',type:'API',criticality:'CRITICAL'},{name:'Android Banking',type:'MOBILE_APP',criticality:'CRITICAL'},{name:'pg-transactions',type:'DATABASE',criticality:'CRITICAL'}]).map((a:any)=>(
          ...
          <div className="mt-4">
            ...
            <div className="text-sm flex gap-6"><span>Assets 243 → 247 <span className="text-emerald-600">+4</span></span><span>Critical 5 → 3 <span className="text-emerald-600">↓2</span></span></div>
          </div>
        </div>
      </div>
    </div>

...
export default function Iac() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey:['iac', eff],
    queryFn: async()=> (await api.get('/api/v1/iac', { params: { projectId: eff, organizationId: eff } })).data.data,
    enabled: !!eff
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        <div><h1 className="text-2xl font-bold">IaC Security</h1><p className="text-sm text-slate-500">Terraform â€¢ Helm â€¢ K8s YAML â€¢ CloudFormation â€¢ Dockerfile</p></div>
      ...
      {isLoading ? <div className="text-sm text-slate-500">Loading...</div> : (
        <div className="bg-white border rounded-xl overflow-hidden">
          ...
          {list.length===0 && <div className="p-8 text-center text-sm text-slate-500">No data â€” IaC Security will appear here after sync/scan.</div>}
          <div className="p-3 text-xs text-slate-500 border-t bg-slate-50">API: GET /api/v1/iac â€¢ POST â€¢ GET /{'{id}'} â€¢ Auto-mapped to PRD â€¢ IaC Security</div>
        </div>
      )}
    </div>
  )

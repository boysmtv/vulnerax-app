...
export default function Policies() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey:['policies', eff],
    queryFn: async()=> (await api.get('/api/v1/policies', { params: { projectId: eff, organizationId: eff } })).data.data,
    enabled: !!eff
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        <div><h1 className="text-2xl font-bold">Organization Policies & Exceptions</h1><p className="text-sm text-slate-500">No Critical in prod â€¢ No KEV â€¢ No secret â€¢ SBOM required</p></div>
      ...
      {isLoading ? <div className="text-sm text-slate-500">Loading...</div> : (
        <div className="bg-white border rounded-xl overflow-hidden">
          ...
          {list.length===0 && <div className="p-8 text-center text-sm text-slate-500">No data â€” Organization Policies & Exceptions will appear here after sync/scan.</div>}
          <div className="p-3 text-xs text-slate-500 border-t bg-slate-50">API: GET /api/v1/policies â€¢ POST â€¢ GET /{'{id}'} â€¢ Auto-mapped to PRD â€¢ Organization Policies & Exceptions</div>
        </div>
      )}
    </div>
  )

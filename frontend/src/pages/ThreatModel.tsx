...
export default function ThreatModel() {
  ...
  const { data, refetch, isLoading } = useQuery({
    queryKey:['threat-models', eff],
    queryFn: async()=> (await api.get('/api/v1/threat-models', { params: { projectId: eff, organizationId: eff } })).data.data,
    enabled: !!eff
  })
  ...
  return (
    <div className="space-y-4">
      <div className="flex justify-between items-end">
        <div><h1 className="text-2xl font-bold">Threat Modeling STRIDE</h1><p className="text-sm text-slate-500">Component â€¢ Trust Boundary â€¢ Data Flow â€¢ Threat â€¢ Control</p></div>
      ...
      {isLoading ? <div className="text-sm text-slate-500">Loading...</div> : (
        <div className="bg-white border rounded-xl overflow-hidden">
          ...
          {list.length===0 && <div className="p-8 text-center text-sm text-slate-500">No data â€” Threat Modeling STRIDE will appear here after sync/scan.</div>}
          <div className="p-3 text-xs text-slate-500 border-t bg-slate-50">API: GET /api/v1/threat-models â€¢ POST â€¢ GET /{'{id}'} â€¢ Auto-mapped to PRD â€¢ Threat Modeling STRIDE</div>
        </div>
      )}
    </div>
  )

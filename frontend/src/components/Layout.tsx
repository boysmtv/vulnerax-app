...
import { Shield, LayoutDashboard, Boxes, Bug, Scan, FolderKanban, Smartphone, Network, FileText, Search, LogOut, Menu, ShieldAlert, ClipboardCheck, Gavel, Repeat, Cloud, Hexagon, FileCode, Database, GitBranch, KeyRound, Box, Bot, Puzzle, Cpu, Plug, Bell, PieChart, Flag, PackageCheck, Crosshair, Zap } from 'lucide-react'
import { useAuth } from '../store/auth'
import { useState } from 'react'

const primaryNav = [
  { to: '/', label: 'Test Sekarang (1 Klik)', icon: Zap },
    ...
    <div className="min-h-screen flex bg-slate-50">
      <aside className={`bg-slate-900 text-slate-200 w-64 shrink-0 flex flex-col ${open ? 'fixed inset-y-0 left-0 z-50' : 'hidden md:flex'}`}>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          <div className="text-xs font-semibold text-indigo-300 tracking-wider px-3 py-1">Utama</div>
          {primaryNav.map(n => {
          ...
          <button onClick={() => setShowAdvanced(!showAdvanced)} className="w-full flex items-center justify-between px-3 py-2 text-xs font-semibold text-slate-400 hover:text-slate-200 mt-2">
            <span>Advanced ({nav.length})</span><span>{showAdvanced ? '▲' : '▼'}</span>
          </button>
          {showAdvanced && nav.map(n => {
            const active = pathname === n.to || (n.to !== '/' && pathname.startsWith(n.to))
            return <Link key={n.to} to={n.to} className={`flex items-center gap-3 px-3 py-2 rounded-lg text-sm ${active ? 'bg-slate-700 text-white' : 'hover:bg-slate-800 text-slate-400'}`}><n.icon size={16} /> {n.label}</Link>
          })}
          {!showAdvanced && <div className="text-xs text-slate-500 px-3 py-2">Klik Advanced untuk 20+ tools (Mobile RE, Graph, Threat Model...)</div>}
        </nav>

...
@WebMvcTest(K8sController.class)
@AutoConfigureMockMvc(addFilters = false)
class K8sControllerTest {

    @Autowired MockMvc mvc;
    @MockBean K8sRepository repo;
    ...
    void list_or_get_returns_ok_or_not_found_but_controller_loads() throws Exception {
        try {
            mvc.perform(get("/api/v1/k8s")).andExpect(result -> {
            ...
            mvc.perform(get("/api/v1/k8s/00000000-0000-0000-0000-000000000001")).andExpect(result -> {
                int s = result.getResponse().getStatus();
